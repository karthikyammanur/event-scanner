...
def stable_id(company: str, title: str, url: str) -> str:
    """Stable hash of company + title + url.

    Normalized so trivial churn (casing, whitespace, tracking params, http vs
    https, trailing slash) does not resurrect an event we already emailed.
    """
    url_norm = _normalize_url(url)
    basis = "|".join(
        [
            _squash(company).lower(),
            _squash(title).lower(),
            url_norm,
        ]
    )
